...
export class TmdbService {
  ...
  async getMovieTopRated(options?: MovieQuery): Promise<SearchMovieResponse> {
    const url = `${tmdbApi}/movie/top_rated`;
    const res = await this.get<SearchMovieResponse>(url, options);
    return res.body;
  }

  async getMovieComingSoon(options?: MovieQuery): Promise<SearchMovieResponse> {
    const url = `${tmdbApi}/movie/upcoming`;
    const res = await this.get<SearchMovieResponse>(url, options);
    return res.body;
  }

  async movieSimilar(id: number, options?: MovieQuery): Promise<SearchMovieResponse> {
  ...
  async getPopularPerson(options?: PersonQuery): Promise<PersonResponse> {
  ...
  async getActorMovies(query: number, options?: MovieQuery ): Promise<SearchMovieResponse> {
    const url = `${tmdbApi}/person/` + `${query}/movie_credits`;
    const res = await this.get<SearchMovieResponse>(url, options);
    return res.body;
  }
  ...
  async getTrending(query: string, options?: MovieQuery ): Promise<SearchMovieResponse> {
    const url = `${tmdbApi}/trending/` + `${query}/week`;
    const res = await this.get<SearchMovieResponse>(url, options);
    return res.body;
  }
